...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof itercumeanabs, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object which iteratively computes a cumulative arithmetic mean of absolute values', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [ 2.0/1.0, 5.0/2.0, 7.0/3.0, 11.0/4.0, 14.0/5.0, 18.0/6.0 ];

	it = itercumeanabs( array2iterator( values ) );
...
tape( 'the function returns an iterator protocol-compliant object which iteratively computes a cumulative arithmetic mean of absolute values (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	it = itercumeanabs( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'if an environment supports `Symbol.iterator` and the provided iterator is iterable, the returned iterator is iterable', function test( t ) {
	...
	itercumeanabs = proxyquire( './../lib/main.js', {
